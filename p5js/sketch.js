/* 
 * GLITCH IS SKIN 
 * High-Visibility Swarm Edition: 1/4 Clones, Enhanced Density, Larger Navigator
 */
...
let pointDensity = 8; // INCREASED DENSITY: More dots for better silhouette
...
let isTouching = false; // NEW FLAG
const ASSEMBLY_HOLD_TIME = 2000; 
let touchStartTime = 0; // NEW TIMER
...
function draw() {
  ...
  // Render Persistent Swarm (Chain following pattern)
  for (let i = 0; i < persistentClones.length; i++) {
    ...
    if (i === 0) {
      ...
      lx = persistentClones[i-1].x;
      ly = persistentClones[i-1].y;
    }
...
class MiniClone {
  constructor() {
    this.x = 0; this.y = 0;
    this.size = 1/3; // LARGER: 1/3 size
    this.indices = [];
    this.history = []; 
    for (let i = 0; i < particles.length; i++) {
      if (random() < 0.25) this.indices.push(i);
    }
  ...
  update(lx, ly) {
    ...
    let tx, ty;
    if (isAssembling) {
      tx = this.formX; ty = this.formY;
    } else {
      // BIRD FLIGHT: Follow leader with a fluid, sensual delay
    ...
    // Sensual tracking: lower lerp for longer trailing chains
  ...
  display() {
    ...
    for (let i = 0; i < this.history.length; i++) {
      ...
      push();
      translate(h.x * 3, h.y * 3); // Scaled for 1/3
      stroke(140, 170, 255, alpha);
      strokeWeight(3); 
    ...
    if (showGlow) {
      ...
      stroke(200, 230, 255, 80 * pulse);
      strokeWeight(bloomSize); 
      for (let idx of this.indices) { if (random() < 0.2) point(particles[idx].x, particles[idx].y); }
      stroke(255, 255, 255, 150 + (105 * pulse));
      strokeWeight(map(pulse, 0, 1, 10, 25)); 
      for (let idx of this.indices) { if (random() < 0.4) point(particles[idx].x, particles[idx].y); }
    }

    stroke(255, 255, 255, showGlow ? 255 : 120); 
    strokeWeight(showGlow ? 8 : 5); 
    for (let idx of this.indices) { point(particles[idx].x, particles[idx].y); }
    pop();
  }
}
...
function drawMainAvatar() {
  ...
  for (let p of particles) {
    // Intense glow when assembling/colliding
    if (showGlow) {
       if (random() < 0.1) {
         // Giant soft bloom
         stroke(180, 220, 255, 20 + 40 * pulse);
         strokeWeight(p.baseSize * 15 * pulse);
         point(p.x, p.y);
       }
       if (random() < 0.2) {
         // Sharp white flare
         stroke(255, 255, 255, 100 + 155 * pulse);
         strokeWeight(p.baseSize * 6 * pulse);
         point(p.x, p.y);
       }
    } else {
      // Standard subtle idle sparkle
      if (random() < 0.03) { 
        stroke(200, 230, 255, 20 + 20 * pulse); 
        strokeWeight(p.baseSize * 2); 
        point(p.x, p.y); 
      }
    }
    p.update(); p.display();
...
function updateStatusUI() {
  ...
  else if (isAssembling) stateText = "ASSEMBLED";
  else if (isTouching) stateText = "COLLIDING";
