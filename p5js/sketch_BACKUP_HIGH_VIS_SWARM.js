/* 
 * GLITCH IS SKIN - BACKUP (HIGH-VISIBILITY SWARM)
 * Saved on: Monday, March 16, 2026
 * 
 * Features:
 * - 1/4 Size mini-clones with high-density point sampling (25%)
 * - Denser main silhouette (pointDensity = 8)
 * - Large pointillist joystick navigator in bottom-right
 * - White & Silver UI theme
 * - Immediate SW Reconstitution with Ease-In-Quad speed
 */
...
function draw() {
  ...
  for (let c of persistentClones) { c.update(); c.display(); }
...
class MiniClone {
  ...
  update() {
    ...
    if (isAssembling) { tx = this.formX; ty = this.formY; } 
      ...
      let noiseX = (noise(frameCount * 0.02, this.driftSeed) - 0.5) * 10;
      let noiseY = (noise(this.driftSeed, frameCount * 0.02) - 0.5) * 10;
      tx = normX * (width * 0.5) + this.offsetX + noiseX;
      ty = normY * (height * 0.5) + this.offsetY + noiseY;
    }
  ...
  display() {
    ...
    for (let i = 0; i < this.history.length; i++) {
      ...
      let alpha = map(i, 0, this.history.length, 0, 50);
      push();
      translate(h.x * 4, h.y * 4); 
...
function drawMainAvatar() {
  for (let p of particles) {
    if (random() < 0.03) { stroke(200, 230, 255, 20); strokeWeight(p.baseSize * 2); point(p.x, p.y); }
    p.update(); p.display();
  }
}
...
function updateStatusUI() {
  ...
  let stateText = isAssembling ? "ASSEMBLED" : (currentStage !== RUPTURE_STAGES.IDLE ? "RUPTURED" : "COHERENT");
