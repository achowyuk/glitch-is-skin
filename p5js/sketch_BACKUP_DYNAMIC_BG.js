/**
 * GLITCH IS SKIN - BACKUP (DYNAMIC BG & SKIN-TO-BLACK)
 * Saved on: Monday, March 16, 2026
 * 
 * Features:
 * - Rotates through 40+ background assets on joystick direction change.
 * - Integration with BackgroundManager for skin-to-black pixelation.
 * - Direction-specific glitching and drifting shadow silhouettes.
 */
    ...
    if (serial && serial.opened() && serial.available() > 0) {
      ...
      if (data && data.trim()) {
        ...
        if (parts.length === 3) {
          ...
          updateDirection();
          if (currentDir !== lastDir && currentDir !== DIRS.NONE) bgManager.next();
          lastDir = currentDir;
        }
      }
    }
...
function handleDriftSpawning() {
  ...
  let now = millis();
  let interval = (glitchIntensity > 0.8) ? 300 : 800;
...
class DriftGhost {
  constructor(vx, vy, sizeMod) {
    this.x = 0; this.y = 0; this.vx = vx; this.vy = vy;
    this.sizeMod = sizeMod; this.alpha = 150;
    this.fadeRate = random(1, 3);
    this.indices = [];
    for (let i = 0; i < particles.length; i++) if (random() < 0.2) this.indices.push(i);
  }
  update() { this.x += this.vx; this.y += this.vy; this.alpha -= this.fadeRate; }
  display() {
    push(); translate(this.x, this.y); scale(this.sizeMod);
    for (let idx of this.indices) {
      let p = particles[idx];
      stroke(200, 200, 255, this.alpha * 0.3);
      strokeWeight(p.baseSize); point(p.x, p.y);
    }
    pop();
  }
}
...
class Particle {
  constructor(x, y, imgW, imgH) {
    this.originX = x - imgW/2; this.originY = y - imgH/2;
  ...
  display(offsetX = 0, offsetY = 0, alpha = 255, isGhost = false) {
    ...
    push(); strokeWeight(this.baseSize);
    ...
    let c = this.baseColor; stroke(red(c), green(c), blue(c), alpha); point(px, py); pop();
  }
}
...
function drawGhosts() {
  ...
  for (let i = 1; i <= numGhosts; i++) {
    ...
    let offX = normX * i * 60; let offY = normY * i * 60;
...
function initiateRupture() {
  ...
  for (let p of particles) {
    ...
    else { let ang = random(TWO_PI); let mag = random(5, 15); p.vx = cos(ang) * mag; p.vy = sin(ang) * mag - 5; }
  }
}
...
function drawFrayingEdges() {
  noFill();
  for (let i = 0; i < 50; i += 5) { stroke(200, 200, 255, map(i, 0, 50, 15, 0)); rect(i, i, width - i*2, height - i*2); }
}

function drawJoystickIndicator() {
  push(); translate(width - 60, height - 60); noFill(); stroke(255, 50); ellipse(0, 0, 40, 40);
  fill(255, 150); noStroke(); ellipse(normX * 15, normY * 15, 8, 8); pop();
}
